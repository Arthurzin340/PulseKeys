...
function ensureAdminAccount(){
  ...
  users.arthur={name:"Arthur",password:"1234",best:Number(old.best||0),admin:true};
  saveUsers(users);
}
ensureAdminAccount();
...
function currentSong(){return playlists[levelKey].find(s=>s.key===songKey)||playlists[levelKey][0]}
...
function stopAudio(){if(melodyTimer)clearTimeout(melodyTimer);melodyTimer=null}
...
function startGame(){
  if(typeof pkUnlocked==="function" && !pkUnlocked(levelKey)){ alert(pkUnlockText(levelKey)); return; }
  ...
  try{
    if(typeof audioCtx!=="undefined" && audioCtx){
      ...
      if(typeof master!=="undefined" && master && audioCtx.currentTime){
        const v = typeof volumeSlider!=="undefined" && volumeSlider ? Number(volumeSlider.value)/100 : 0.85;
        master.gain.cancelScheduledValues(audioCtx.currentTime);
        master.gain.setTargetAtTime(Math.max(0,Math.min(1,v))*0.95, audioCtx.currentTime, 0.015);
      }
    }
  }catch(e){}
}
